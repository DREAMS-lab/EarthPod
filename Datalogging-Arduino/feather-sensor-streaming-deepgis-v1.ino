...
WiFiClient client;
HttpClient http(client, "sensors.deepgis.org", 80);
...
void loop() {  
  ...
  String get_str = "/earthpoddata?id=" + String(2) +"&atmos_temperature=" + String(temp_sht21)+"&atmos_relative_humidity=" + String(rh)+"&soil_relative_humidity=" + String(AM2315_hum0)+"&soil_temperature=" + String(AM2315_temp0);
  int err = 0;
  Serial.println(String(get_str));

  err = http.get(get_str);
  if (err == 0)
  {
